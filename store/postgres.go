...
func (p *pg) UpdateDetails(ctx context.Context, in *objects.UpdateDetailsRequest) error {
	...
	return p.db.WithContext(ctx).Model(evt).Select(
		"name", "description", "website", "address", "phone_number", "updated_on").
		Updates(evt).Error
}
...
func (p *pg) Delete(ctx context.Context, in *objects.DeleteRequest) error {
	...
	return p.db.WithContext(ctx).Model(evt).Delete(evt).Error
}
